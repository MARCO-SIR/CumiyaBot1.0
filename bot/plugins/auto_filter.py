...
async def auto_filter(bot: Client, update: Message):
    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            if file_size < 1024:
                ...
                file_size = f"[{str(round(file_size/1024, 2))} K.B] "
            elif file_size < (1024**3):
                file_size = f"[{str(round(file_size/(1024**2), 2))} M.B] "
            elif file_size < (1024**4):
                file_size = f"[{str(round(file_size/(1024**3), 2))} G.B] "
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("➡️Move to next page➡️", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"👇 Check showed links 👇 {(len_results)} Results For Your command 🤗: <code>{query}</code>",
